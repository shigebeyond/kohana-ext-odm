	...
	public static function element($type, $name, $value, $params)
	{
		...
		switch ($type) // 输出type对应的控件
		{
			...
			case 'select':
				...
				{
					$options = Mongo_Document::factory('Enum', $options)->enarray();
				}
			...
			case 'enum':
				...
				if (is_string($options)) // 构建枚举
				{
					return Mongo_Document::factory('Enum', $options)->envalue($value);
				}
			...
			case 'fileinfo':
				//$file = Mongo_Document::factory('File')->where('md5', '=', $value)->find();
				$file = Mongo_Document::factory('File', $value);
